...
Row filters let you attach row-level predicates to a schema so that SELECT/UPDATE/DELETE automatically include them. This is useful for authorization scoping in systems like Permiso, and can provide an RLS-like safety net even on databases without native RLS support.
...
```typescript
const schemaWithPolicies = dangerousUnrestrictedSchema.withRowFilters<PermisoContext>({
  users: {
